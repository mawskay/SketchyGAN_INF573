...
brush = cv2.imread("brush.png",3)#paint brush icon
eraser = cv2.imread("model.png",3)#eraser icon
plus = cv2.imread("plus.png",3)#to reduce the brush or eraser size
minus = cv2.imread("minus.png",3)# to increase the brush or eraser size
...
canvas[0:20,0:20] = cv2.resize(brush, (20,20), interpolation = cv2.INTER_AREA)
#canvas[0:20,40:60] = cv2.resize(eraser, (20,20), interpolation = cv2.INTER_AREA)
#canvas[0:20,80:100] = (0,0,255) #red color
canvas[0:20,120:140] = (0,255,0)#green color
canvas[0:20,160:180] = (0,0,255)#red color
canvas[0:20,200:220] = cv2.resize(plus, (20,20), interpolation = cv2.INTER_AREA)
canvas[0:20,240:260] = cv2.resize(minus, (20,20), interpolation = cv2.INTER_AREA)
...
def click(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if y in range(0,20) and x in range(120,140):#to change color of brush to red
            color = (0,0,0)
            generate_and_show(canvas[40:,:], 1)
            #open_cv_image = numpy.array(pil_image) 
            # Convert RGB to BGR 
            #open_cv_image = open_cv_image[:, :, ::-1].copy() 
            #cv2.imwrite('canvas.jpg', canvas[40:,:])
        if y in range(0,20) and x in range(160,180):#to change color of brush to red
            color = (0,0,0)
            generate_and_show(canvas[40:,:], 0)
